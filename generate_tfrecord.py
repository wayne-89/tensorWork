...
flags.DEFINE_string('label_map_path', '', 'Path to the label map')
FLAGS = flags.FLAGS
...
def create_tf_example(group, path, labelMap):
    ...
    for index, row in group.object.iterrows():
        ...
        if labelMap is not None:
            classes.append(int(labelMap[row['class']]))
        else:
...
def main(_):
    ...
    if labelMapPath is not None:
        labelSection="label"
        cf = configparser.ConfigParser()
        cf.read(labelMapPath)
        labels=cf.items(labelSection)
        labelMap={}
        for label in labels:
            labelMap[label[1]]=label[0]
        print('dd',labelMap)
